...
export default async (req:IRequest, resp:Response, next:NextFunction) =>{
    ...
    // Нету проверки, что токен сделан конкретно тобой jwt.verify(token,sekretkey)
    ...
    if(!user.verifyUser.isVerify){
        const error: ApiError = ApiError.forbidden("your account dont vertify")
        return resp.status(error.status).json({error:error.msg});
    }
